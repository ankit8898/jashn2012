...
class JashnFunkyMessageValidator < ActiveModel::Validator
  def validate(record)
    if record.name.blank? && record.sex.blank? && record.cell_num.blank? && record.email_id.blank?
      record.errors[:base] << "We cannot move ahead without your Name, Sex , Email and Mobile No."
